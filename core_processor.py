...
**在开始生成笔记之前，你必须首先扫描全文，提取以下关键信息。这些信息对学生至关重要，遗漏可能导致严重后果！**

请在笔记开头（第0部分）单独列出以下信息（如果讲座中提及）：

## 📢 课程重要通知 【必读】

> ⚠️ **如果老师在讲座中提到了以下任何内容，必须在此处醒目标注！**

### 🎯 作业相关
- **作业布置**: [作业内容、要求]
- **截止时间**: [具体日期时间，用 🔴 标注紧急程度]
- **提交方式**: [如何提交、提交到哪里]
- **注意事项**: [格式要求、命名规范、扣分规则等]

### 📝 考试相关
- **考试安排**: [考试时间、地点、形式（开卷/闭卷）]
- **考试范围**: [具体章节、重点内容]
- **题型说明**: [选择题、计算题、简答题等比例]
- **老师特别提醒**: [老师强调的考试重点、必考内容]
...
**不同课程有不同的知识特点和考查重点，你必须根据课程性质调整笔记的侧重点，不能一刀切！**

## 课程类型识别与应对策略

### 📊 计算密集型课程
**典型代表**：高等数学、线性代数、概率论、数值分析、信号与系统
**核心特点**：公式推导、计算技巧、解题方法
**笔记侧重**：
- ✅ 详细记录计算步骤和推导过程
- ✅ 标注计算技巧和易错点
- ✅ 整理典型例题和解法套路
- ❌ 不要过度展开概念解释（计算课的概念通常较简单）

### 📚 概念理解型课程
**典型代表**：操作系统、计算机网络、数据库原理、编译原理、计算机组成原理
**核心特点**：概念定义、机制原理、设计思想、特性对比
**笔记侧重**：
- ✅ **详细记录每个概念的完整定义和特性**（例如：调度算法的所有评价指标、适用场景、优缺点）
- ✅ **机制的"为什么这样设计"比"怎么计算"更重要**
- ✅ 特性对比表（如：不同调度算法的特性对比）
- ✅ 设计权衡和取舍（如：为什么选择这种方案而不是另一种）
- ❌ 不要只关注计算例题而忽略概念特性

### 🧠 记忆背诵型课程
**典型代表**：计算机网络（协议细节）、法律法规、历史、部分专业课
**核心特点**：大量需要记忆的事实、规定、特征
**笔记侧重**：
- ✅ 分类整理、表格对比
- ✅ 记忆口诀、联想技巧
- ✅ 高频考点标注
...
### 🔬 理论推导型课程
**典型代表**：离散数学、算法设计、形式语言与自动机
**核心特点**：定理证明、逻辑推导、形式化描述
**笔记侧重**：
- ✅ 完整的证明过程
- ✅ 定理之间的逻辑关系
- ✅ 反例和边界情况
...
1. **不要用计算型思维处理概念型课程**
   - ❌ 错误示例（操作系统）：只关注"周转时间怎么算"，忽略"SJF算法的饥饿问题、适用场景、与FCFS的对比"
   - ✅ 正确做法：概念的**特性、优缺点、适用场景**和计算方法**同等重要**

2. **不要遗漏老师强调的"非计算内容"**
   - 老师说"这个特性很重要"、"这里经常考"→ 必须详细记录
   - 老师花时间讲解的对比、区别、特点 → 必须完整保留

3. **不要用自己的理解替代老师的讲授重点**
   - 老师强调的就是重点，即使你认为不重要
   - 老师一笔带过的可以简略，老师反复强调的必须详细
...
4. **忠实**：以老师的讲授重点为准，不要自作主张调整重点

请严格按照以下部分输出，使用指定的 Markdown 标题。

---

# 0. 课程重要通知 【如有则必须首先输出】

（按上述「课程事务性信息提取」的格式输出，如无则写「本节课无特殊通知」）
...
- ❌ **禁止只关注计算**：概念的特性、优缺点、适用场景与计算同等重要！
- ✅ 必须有深度：像教科书一样详细解释
- ✅ 必须标重点：【重点】【难点】【易错】【考点】明确标注
- ✅ **必须忠于老师的讲授重点**：老师强调什么，你就重点写什么
...
**学习目标**: [列出3-5个具体的学习目标]
**课程类型判断**: [计算密集型/概念理解型/记忆背诵型/理论推导型/实践应用型/混合型]
...
**本节侧重点判断**：
- 老师在本节更侧重：[ ] 计算/推导  [ ] 概念理解  [ ] 特性对比  [ ] 设计原理  [ ] 记忆背诵
- 因此笔记应侧重：[根据判断调整侧重点]
...
**⚠️ 如果本节涉及多个可对比的概念/方法/算法，必须制作完整的对比表！**
...
**⚠️ 本节记录老师在讲课中明确强调的内容，原汁原味保留老师的表述！**

#### 10.1 老师明确说"重要/必考/经常考"的内容
- 「...」（老师原话或近似表述）→ 对应知识点：...
- 「...」→ 对应知识点：...
...
class CoreProcessor:
    ...
    def _generate_note(self, contents):
        ...
        # 情况1：第二次结果包含完成标记 → 说明续写成功，需要拼接
        if COMPLETION_MARKER in second_result:
            logger.info(f"✅ 第二次生成包含 '{COMPLETION_MARKER}'，执行拼接")
            merged_result = self._merge_continuations(first_result, second_result)
            logger.info(f"✅ 拼接完成，最终字符数: {len(merged_result)}")
            return merged_result
        
        # 情况2：第二次结果不包含完成标记，但内容较多 → 可能是部分续写，仍然拼接
        if len(second_result) >= 500:
            logger.warning(f"⚠️ 第二次生成未包含 '{COMPLETION_MARKER}'，但内容较多（{len(second_result)}字符），尝试拼接")
            merged_result = self._merge_continuations(first_result, second_result)
            logger.info(f"✅ 拼接完成，最终字符数: {len(merged_result)}")
