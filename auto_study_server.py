        ...
        try:
            if task_type == 'note':
                ...
                if payload.get("cancelled"):
                    ...
                    update_task(task_id, "error", 0, "任务执行失败，请查看日志")
                else:
                    note_path = payload.get("note_path")
                    subtitle_path = payload.get("subtitle_path")
                    message = "笔记生成完成"
                    if note_path:
                        message += f"，笔记: {note_path}"
                    update_task(task_id, "completed", 100, message)
                    with task_lock:
                        if task_id in task_status:
                            if note_path:
                                task_status[task_id]["notePath"] = note_path
                            if subtitle_path:
                                task_status[task_id]["subtitlePath"] = subtitle_path
                            if payload.get("transcript_path"):
                                task_status[task_id]["transcriptPath"] = payload["transcript_path"]
                ...
                update_task(task_id, "processing", 10, "正在转录音频...")
                ...
                try:
                    ...
                    update_task(task_id, "completed", 100, f"转录完成！\n字幕: {subtitle_path}")
                    
                    with task_lock:
                        if task_id in task_status:
                            task_status[task_id]["subtitlePath"] = subtitle_path
                            task_status[task_id]["transcriptPath"] = transcript_path
                            
                except Exception as e:
                    logger.error(f"转录失败: {e}")
                    update_task(task_id, "error", 0, f"转录失败: {str(e)}")
            
            elif task_type == 'generate-note':
                # 仅生成笔记任务
                course_name = task['courseName']
                lesson_title = task['lessonTitle']
                subtitle_path = task['subtitlePath']
                video_url = task.get('videoUrl', '')
                
                update_task(task_id, "processing", 20, "正在读取字幕...")
                ...
                try:
                    ...
                    update_task(task_id, "processing", 40, "正在调用 Gemini 生成笔记...")
                    ...
                    update_task(task_id, "processing", 80, "正在保存笔记...")
                    ...
                    update_task(task_id, "completed", 100, f"笔记生成完成！\n笔记: {note_path}")
                    ...
                    update_task(task_id, "error", 0, f"笔记生成失败: {str(e)}")
            
            elif task_type == 'download':
                # 下载逻辑也复用 CoreProcessor (部分复用)
                url = task['url']
                filename = task['filename']
                file_type = task['fileType']
                
                update_task(task_id, "downloading", 10, "准备下载...")
                
                formatted_name = processor.parse_metadata(filename, filename)
                ext = "m4a" if file_type == 'audio' else "mp4"
                
                # 使用 core_processor 中定义的 DOWNLOAD_DIR
                # 为了兼容性，这里可以重新读取配置或传递
                # 简单起见，我们直接拼接路径
                from core_processor import DOWNLOAD_DIR
                os.makedirs(DOWNLOAD_DIR, exist_ok=True)
                output_path = os.path.join(DOWNLOAD_DIR, f"{formatted_name}.{ext}")
                
                if should_cancel():
                    update_task(task_id, "cancelled", 0, "任务已取消")
                    continue

                success = processor.download_media(
                    url, 
                    output_path, 
                    audio_only=(file_type == 'audio')
                )
                ...
                if not success:
                    if should_cancel():
                        ...
                        update_task(task_id, "error", 0, "下载失败")
                    continue
                
                completion_msg = f"已保存至: {output_path}"
                update_task(task_id, "completed", 100, completion_msg)
                
                # 记录下载路径
                with task_lock:
                    if task_id in task_status:
                        task_status[task_id]["audioPath"] = output_path if file_type == 'audio' else None
                        task_status[task_id]["videoPath"] = output_path if file_type == 'video' else None
